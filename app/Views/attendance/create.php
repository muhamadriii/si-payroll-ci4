...
<?= $this->section('title') ?>Input Absensi Bulan<?= $this->endSection() ?>
<?= $this->section('title_page') ?>Rekap Presensi<?= $this->endSection() ?>
<?= $this->section('breadcrumbs') ?>Transaksi / Absensi / Rekap<?= $this->endSection() ?>
  ...
  <div class="card-body">
    <form method="post" action="<?= site_url('attendance/store') ?>">
      ...
      <div class="table-responsive">
        <table class="table table-striped border" id="createAttendanceTable">
          <thead>
            <tr>
              <th>NIK</th>
              <th>Username</th>
...
<script>
$(function(){
  $('#createAttendanceTable').DataTable({
    pageLength: 10,
